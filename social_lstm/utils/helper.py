...
from social_lstm.utils.model import SocialModel
from social_lstm.utils.olstm_model import OLSTMModel
from social_lstm.utils.vlstm_model import VLSTMModel
import argparse
import pandas as pd
import pickle
...
class WriteOnceDict(dict):
    def __setitem__(self, key, value):
        """
        Sets a key-value pair in the dictionary. If the key does not already exist in the dictionary, 
        it adds the key-value pair. If the key already exists, it does not modify the existing value.
...
def get_method_name(index):
    """
    Returns the method name given the index.

    Parameters:
    index (int) : Index of the method (1 = Social LSTM, 2 = Obstacle LSTM, 3 = Vanilla LSTM)
...
def get_model(index, arguments, infer = False):
    """
    Returns a model given the index and arguments.

    Parameters:
    index (int) : Index of the model (1 = Social LSTM, 2 = Obstacle LSTM, 3 = Vanilla LSTM)
    arguments (dict) : Arguments to pass to the model
    infer (bool, optional) : Whether to use the model for inference or training. Defaults to False.
...
def get_args():
    ...
    Parameters:
    - input_size (int): size of input data (default: 2)
    - output_size (int): size of output data (default: 5)
    - rnn_size (int): size of RNNs (default: 128)
    - embedding_size (int): size of embedding layer (default: 64)
    - grid_size (int): size of grid (default: 4)
    - maxNumPeds (int): maximum number of pedestrians in a frame (default: 27)
    - dropout (float): dropout rate for RNNs (default: 0.5)
    - use_cuda (bool): whether to use CUDA or not (default: True if available)
    - obs_length (int): observed length of the trajectory (default: 8)
    - pred_length (int): predicted length of the trajectory (default: 12)
    - epoch (int): epoch of the model to be loaded (default: 24)
    - gru (bool): whether to use GRU cell or LSTM cell (default: False)
    - method (int): type of LSTM method (default: 1)
        1: social LSTM
        2: obstacle LSTM
        3: vanilla LSTM
    Returns:
