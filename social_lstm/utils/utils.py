...
class DataLoader():
    ...
    def convert_proper_array(self, x_seq, pedlist):
    ...
    def get_directory_name_with_pointer(self, pointer_index):
        # Get full file path
        full_path = self.data_dirs[pointer_index]
        # Get the dataset folder name (e.g., 'biwi') from the parent directory
    ...
    def write_dataset(self, dataset_seq, file_name, path):
        ...
        full_output_path = os.path.join(normalized_path, file_name)

        # Ensure the full directory path exists (including subfolders from file_name)
        os.makedirs(os.path.dirname(full_output_path), exist_ok=True)

        # Log output location
        print(f"Writing to file path: {normalized_path}, file_name: {file_name}")

        out = np.concatenate(dataset_seq, axis=0)
        np.savetxt(full_output_path, out, fmt="%1d %1.1f %.3f %.3f", newline='\n')
