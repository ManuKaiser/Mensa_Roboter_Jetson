...
import numpy as np
from social_lstm.utils.utils import DataLoader
from social_lstm.utils.helper import getCoef, sample_gaussian_2d, get_mean_error, get_final_error
from social_lstm.utils.helper import *
from social_lstm.utils.grid import getSequenceGridMask, getGridMask
...
def main():
    ...
    if not os.path.isdir("log/"):
      ...
      subprocess.call(['python', 'make_directories.py'])
    ...
    for iteration in range(sample_args.iteration):
        ...
        for batch in range(dataloader.num_batches):
            ...
            x_seq, lookup_seq = dataloader.convert_proper_array(x_seq, PedsList_seq)
            ...
            if sample_args.method == 3: #vanilla lstm
                ...
                ret_x_seq = sample(obs_traj, obs_PedsList_seq, sample_args, net, PedsList_seq, saved_args, dataset_data, lookup_seq, sample_args.gru)
                ...
                ret_x_seq = sample(obs_traj, obs_PedsList_seq, sample_args, net, PedsList_seq, saved_args, dataset_data, lookup_seq, sample_args.gru, obs_grid)
...
def sample(x_seq, Pedlist, args, net, true_Pedlist, saved_args, dimensions, look_up, is_gru, grid = None):
    '''
    The sample function
    params:
    ...
    with torch.no_grad():
        ...
        for tstep in range(args.obs_length-1):
            if grid is None: #vanilla lstm
               # Do a forward prop
                out_obs, hidden_states, cell_states = net(x_seq[tstep].view(1, numx_seq, 2), hidden_states, cell_states, [Pedlist[tstep]], look_up)
            else:
                # Do a forward prop
                out_obs, hidden_states, cell_states = net(x_seq[tstep].view(1, numx_seq, 2), [grid[tstep]], hidden_states, cell_states, [Pedlist[tstep]], look_up)
        ...
        for tstep in range(args.obs_length-1, args.pred_length + args.obs_length-1):
            # Do a forward prop
            if grid is None: #vanilla lstm
                outputs, hidden_states, cell_states = net(ret_x_seq[tstep].view(1, numx_seq, 2), hidden_states, cell_states, [true_Pedlist[tstep]], look_up)
            else:
                outputs, hidden_states, cell_states = net(ret_x_seq[tstep].view(1, numx_seq, 2), [prev_grid], hidden_states, cell_states, [true_Pedlist[tstep]], look_up)
